...
(function(r,o){typeof exports=="object"&&typeof module<"u"?o(exports):typeof define=="function"&&define.amd?define(["exports"],o):(r=typeof globalThis<"u"?globalThis:r||self,o(r.TextInterface={}))})(this,function(r){"use strict";const o="",l=["yes","yeah","yep","yup","true","t","y","aye","yup"],a=["no","n","false","f","nope","nah"];class h{constructor(t=document.body,e="Text Interface"){this.listener=null,this.outputAnimationLength=800,this.outputDelay=300,this.shouldStealFocus=!1,this.scrollOptions={behavior:"smooth",block:"center",inline:"nearest"},this.outputQueue=[],this.div=document.createElement("div"),this.div.classList.add("text-interface"),t.appendChild(this.div),this.div.innerHTML=`
    ...
    `,this.inputWrap=this.div.querySelector(".input-wrap"),this.inputEl=this.div.querySelector(".input"),this.outputEl=this.div.querySelector(".output"),this.placeholderEl=this.div.querySelector(".placeholder"),this.setupInputListener(),this.shouldStealFocus&&this.inputEl.focus()}setTitle(t){this.div.querySelector(".ti-title").textContent=t}clear(){this.outputEl.innerHTML=""}setScrollOptions(t){this.scrollOptions=t}async readChoice(t,e="Choose one of the following:",i="You must choose one of the options!"){this.output(e);for(let u=0;u<t.length;u++)this.output(`${u+1}. ${t[u]}`);let s=await this.readText();if(t.indexOf(s)>-1)return s;if(s=s.replace(/\D/g,""),s!=""){let u=Number(s);if(!isNaN(u)&&u<=t.length&&u>0)return t[Math.floor(u)-1]}return this.output(i),await this.readChoice(t,e,i)}async readYesOrNo(t="Say yes or no!"){let e=await this.readText();return e=e.toLowerCase(),e=e.replace(/\s+/,""),l.indexOf(e)>-1?!0:a.indexOf(e)>-1?!1:(this.output(t),await this.readYesOrNo(t))}async readNumber(t="Please type a number"){let e=await this.readText(),i=Number(e);return isNaN(i)?(this.output(t),this.readNumber(t)):i}async readInteger(t="Please type a whole number"){let e=await this.readText(),i=Number(e);return isNaN(i)||i%1!=0?(this.output(t),this.readInteger(t)):i}async readIntegerInRange(t,e,i=`Please type a whole number within the range of ${t} and ${e}`){let s=await this.readText(),n=Number(s);return isNaN(n)||n%1!=0||n>e||n<t?(this.output(i),await this.readIntegerInRange(t,e,i)):n}async readNumberInRange(t,e,i=`Please type a whole number within the range of ${t} and ${e}`){let s=await this.readText(),n=Number(s);return isNaN(n)||n>e||n<t?(this.output(i),await this.readNumberInRange(t,e,i)):n}readText(){return this.shouldStealFocus&&this.inputEl.focus(),this.inputWrap.classList.add("active"),this.inputWrap.scrollIntoView(this.scrollOptions),new Promise((t,e)=>{this.listener=t})}prompt(t){return this.output(t),this.readText()}promptYesOrNo(t){return this.output(t),this.readYesOrNo()}promptNumber(t,e){return this.output(t),this.readNumber(e)}promptInteger(t,e){return this.output(t),this.readInteger(e)}promptNumberInRange(t,e,i,s){return this.output(t),this.readNumberInRange(e,i,s)}promptIntegerInRange(t,e,i,s){return this.output(t),this.readIntegerInRange(e,i,s)}showElement(t){this.outputting?this.outputQueue.push(["element",t]):(this.outputting=!0,this.outputEl.appendChild(t),t.scrollIntoView(this.scrollOptions),setTimeout(()=>{this.outputting=!1,this.doNextOutput()},this.outputDelay))}showHTML(t){let e=document.createElement("div");e.innerHTML=t,this.showElement(e)}showImage(t,e="An image"){let i=document.createElement("img");i.setAttribute("src",t),i.setAttribute("alt",e),this.showElement(i)}output(t,e=!1){if(t=""+t,this.outputting)this.outputQueue.push(["text",t,e]);else{let i=document.createElement("div");if(i.classList.add("output"),e&&i.classList.add("echo"),!this.outputAnimationLength||e)i.textContent=t;else{this.outputting=!0;let s=this.outputAnimationLength/t.length;const n=()=>{if(t[0]===`
`){const u=document.createElement("br");i.appendChild(u)}else{const u=document.createTextNode(t[0]||"");i.appendChild(u)}t=t.substring(1),t.length?setTimeout(n,s):(this.outputting=!1,this.doNextOutput())};setTimeout(n,this.outputDelay)}this.outputEl.appendChild(i),i.scrollIntoView(this.scrollOptions)}}doNextOutput(){if(this.outputQueue.length){let t=this.outputQueue[0];this.outputQueue=this.outputQueue.slice(1);let e=t[0],i=t.slice(1);e=="text"?this.output(...i):this.showElement(...i)}}setupInputListener(){this.inputEl.addEventListener("keyup",t=>{if(t.code=="Enter"||t.key=="Enter"){let i=this.inputEl.textContent.replace(/\n$/,"");this.output(i,!0),this.listener&&(this.listener(i),this.listener=null),this.inputWrap.classList.remove("active"),setTimeout(()=>{this.inputEl.textContent=""},1)}}),this.placeholderEl.addEventListener("click",()=>this.inputEl.focus())}}r.TextInterface=h,Object.defineProperty(r,Symbol.toStringTag,{value:"Module"})});
