...
var a = class {
  ...
  async readYesOrNo(t = "Say yes or no!") {
    ...
    return e = e.toLowerCase(), e = e.replace(/\s+/, ""), r.indexOf(e) > -1 ? true : o.indexOf(e) > -1 ? false : (this.output(t), await this.readYesOrNo(t));
  }
  ...
  showElement(t) {
    this.outputting ? this.outputQueue.push(["element", t]) : (this.outputting = true, this.outputEl.appendChild(t), t.scrollIntoView(this.scrollOptions), setTimeout(() => {
      this.outputting = false, this.doNextOutput();
    }, this.outputDelay));
  }
  showHTML(t) {
...
`) {
          ...
          t = t.substring(1), t.length ? setTimeout(n, s) : (this.outputting = false, this.doNextOutput());
        };
  ...
  setupInputListener() {
    this.inputEl.addEventListener("keyup", (t) => {
      if (t.code == "Enter" || t.key == "Enter") {
        ...
        this.output(i, true), this.listener && (this.listener(i), this.listener = null), this.inputWrap.classList.remove("active"), setTimeout(() => {
